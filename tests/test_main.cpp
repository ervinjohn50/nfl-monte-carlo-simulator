...
// ---------------------------------------------------------------------
void test_empty_csv_throws() {
    // Create an empty file
    std::ofstream("empty_test.csv").close();

    bool threw = false;
    try {
        load_teams_csv("empty_test.csv");
    } catch (const std::runtime_error&) {
        threw = true;
    }

    // Clean up — delete the file we created
    std::remove("empty_test.csv");

    check(threw, "Loading an empty CSV throws instead of silently returning zero teams");
}
